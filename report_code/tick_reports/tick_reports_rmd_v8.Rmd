...
photo_own <- (i %in% tidy_mammals_photos$Ownership) ## this will be used later to decide whether to evaluate code chunks or not
```
...
if(i %in% tidy_mammals_photos$Ownership) { ## ends at line 60
...
tick_table %>%
  ...
  kable_styling(font_size = ifelse(
    length(subset(ticks_caught_table, Ownership == i)$Site) >= 5 ,
    14, 16))
...
pathogens_table ## print the table stating which pathogens were present at each site
```
...
if(i %in% tidy_mammals_photos$Ownership) { ## if the owner is listed as an owner in the camera data\\ ends at line 242
