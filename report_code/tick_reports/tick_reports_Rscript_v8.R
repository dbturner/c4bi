...
load(here::here("data/tidy_tick_sites.rda"))
load(here::here("data/ticks_caught_table.rda"))
load(here::here("data/ticks_caught.rda"))
load(here::here("data/ticks_caught_county.rda"))
load(here::here("data/tidy_pathogens_long.rda"))
load(here::here("data/tidy_pathogens_county.rda"))
load(here::here("data/pathogens_table_1.rda"))
load(here::here("data/tidy_mammals_species_richness.rda"))
load(here::here("data/tidy_mammals_species_richness_county.rda"))
load(here::here("data/tidy_mammals_photos.rda"))
load(here::here("data/tidy_mammals_each_site.rda"))
...
Owners <- factor(unique(tidy_tick_sites$Ownership))
...
for (i in Owners){ ## ends at line 403
 ...
 #Section 1---------------------------------------------------------------------
  ...
  Sites <- unique(subset(ticks_caught, Ownership == i)$Site)
  levels <- c('Chester','Delaware','Philadelphia',
              unique(subset(ticks_caught,
                            Ownership == i)$Site))

  ## Now assign the ggplot object to a name that can be called in the Rmd
  tick_abundance_plot <- ggplot(data = subset(ticks_caught, Ownership == i),
  ...
  ## now plot the county level bars separately
    geom_bar(data = ticks_caught_county,
    ...
    geom_text(data = subset(ticks_caught, Ownership == i),
    ...
    geom_text(data = ticks_caught_county,
              aes(x = County,
  ...
  pathogens_table <- pathogens_table_1 %>%
    filter(Ownership == i) %>%
    ungroup() %>%
  ...
  county_pathogens <-  ggplot(data = tidy_pathogens_county,
  ...
  ## so we can do conditional formatting within the ggplot()
  Sites <- unique(subset(tidy_mammals_species_richness, Ownership == i)$Site)
  levels <- c('Chester','Delaware','Philadelphia',
              unique(subset(tidy_mammals_species_richness,
                            Ownership == i)$Site))

    wild_species_richness <- ggplot(data = subset(tidy_mammals_species_richness, Ownership == i),
      ...
      stat_summary(data = tidy_mammals_species_richness,
                   aes(x = County, y = county_avg_rich),
                   alpha = 0.5, stat = "identity", fun = "mean", geom = "bar") +
      geom_text(data = subset(tidy_mammals_species_richness, Ownership == i),
                aes(label=species_richness),
                position = position_dodge(width = 0.5),
                vjust = 0.5, hjust = 1,
                size = 5, fontface = "bold") +
      geom_text(data = tidy_mammals_species_richness_county,
                aes(x = County,
                    y = county_avg_rich,
    ...
    Sites <- unique(subset(tidy_mammals_photos, Ownership == i)$Site)
    levels <- c('Chester','Delaware','Philadelphia',
                unique(subset(tidy_mammals_photos,
                              Ownership == i)$Site))


    wild_photo_abundance <- ggplot(data = subset(tidy_mammals_photos, Ownership == i),
                             aes(x = Site, y = species_photos, fill = Species)) +
      geom_bar(stat = "identity", position = "stack") +
      ## using stat_summary gives the same desired output at geom_bar but without the issues
      stat_summary(data = tidy_mammals_photos,
    ...
    wildlife_table <- tidy_mammals_each_site %>%
      filter(Ownership == i) %>%
      ungroup() %>%
    ...
    numSites <- length(unique(subset(tidy_tick_sites, Ownership == i)$Site))
    ## number of sites surveyed specifically for ticks
    numTickSites <- length(unique(subset(ticks_caught, Ownership == i)$Site))
    ## number of sites where camera trapping was done
    numCamSites <- length(unique(subset(tidy_mammals_photos, Ownership == i)$Site))

    ## list of sites surveyed throughout
    listSites <- unique(subset(tidy_tick_sites, Ownership == i)$Site)
    ## list of sites surveyed for ticks
    listTickSites <- unique(subset(ticks_caught, Ownership == i)$Site)
    ## list of sites where cameras were used
    listCamSites <- unique(subset(tidy_mammals_photos, Ownership == i)$Site)
    ...
    render(here("report_code/tick_reports/tick_reports_rmd_v8.Rmd"), output_file = paste0('report_output/tick_report_', i, "v8.html"), "html_document") # This line executes each owner's .html report by calling the "child" .rmd file.
